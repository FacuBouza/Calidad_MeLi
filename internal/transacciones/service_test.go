...
func TestUpdateAndGetSQLMock(t *testing.T) {
	...
	idTransacModificada := 3

	mock.ExpectPrepare("UPDATE transacciones")
	mock.ExpectExec("UPDATE transacciones").WillReturnResult(sqlmock.NewResult(3, 1))

	rows := sqlmock.NewRows([]string{"moneda", "monto", "emisor", "receptor"})
	rows.AddRow("Pesos", 6500.45, "Facundo2", "Matias2")
	mock.ExpectQuery("SELECT moneda, monto, emisor, receptor FROM transacciones").WithArgs(idTransacModificada).WillReturnRows(rows)

	repo := NewRepositorySQLMock(db)
	service := NewServiceSQL(repo)

	//Act
	transacUpdate, err := service.Update(transaccionModificada)
	transacGet := service.GetOne(idTransacModificada)

	//Assert
	assert.Equal(t, transaccionModificada.Emisor, transacUpdate.Emisor)
	assert.Equal(t, transaccionModificada.Receptor, transacUpdate.Receptor)
	assert.Equal(t, transaccionModificada.Monto, transacUpdate.Monto)
	assert.Equal(t, transaccionModificada.Receptor, transacGet.Receptor)
	assert.Equal(t, transaccionModificada.Emisor, transacGet.Emisor)
	assert.Equal(t, transaccionModificada.Monto, transacGet.Monto)
	assert.Nil(t, err)
}

func TestDeleteErrorSQLTxdb(t *testing.T) {
	//Arrange
	db, err := db.InitDb()
	assert.Nil(t, err)
	defer db.Close()
	...
	//Act
	err = service.Delete(inexistentID)
